...
function base64decodebytes(str) {
  ...
  for (var i = 0; i < str.length; i++) {
    var rep = BASE64_ALPHA.indexOf(str[i]);

    if (rep === -1) {
      if (str[i] === '=') break;else throw new Error('unknown character in base64 string');
    }
  ...
  if (tail256 === 1) {
    var _field = (b64[i64] & 63) << 2 | b64[i64 + 1] >> 4 & 63;

    b256[i256] = _field & 255;
  } else if (tail256 === 2) {
    var _field2 = (b64[i64] & 63) << 10 | (b64[i64 + 1] & 63) << 4 | b64[i64 + 2] >> 2 & 63;

    b256[i256++] = _field2 >> 8 & 255;
    b256[i256] = _field2 & 255;
  }
...
function base64encodebytes(b256) {
  ...
  if (tail === 1) {
    var _field3 = b256[i256] & 0xFF;

    b64[i64++] = _field3 >> 2 & 63;
    b64[i64] = (_field3 & 3) << 4;
  } else if (tail === 2) {
    var _field4 = (b256[i256] & 0xFF) << 8 | b256[i256 + 1] & 0xFF;

    b64[i64++] = _field4 >> 10 & 63;
    b64[i64++] = _field4 >> 4 & 63;
    b64[i64] = (_field4 & 15) << 2;
  }

  var s64 = b64.map(function (e) {
    return BASE64_ALPHA[e];
  }).join('');

  if (tail > 0) {
    for (var i = tail; i < 3; i++) {
      s64 += '=';
    }
  }
...
var DerInterval = function DerInterval(tag, skip, count) {
  _classCallCheck(this, DerInterval);
...
function getDerInteger(data, interval) {
  ...
  var arr = data.filter(function (e, i) {
    return i >= interval.valIndex && i - interval.valIndex < interval.valCount;
  });
  return bigInt.fromArray(arr, 256);
}

var DH_DATA_BYTES = [42, 134, 72, 134, 247, 13, 1, 3, 1];
var DH_MODULUS = bigInt('24103124269210325885520760221975' + '66074856950548502459942654116941' + '95810883168261222889009385826134' + '16146732271414779040121965036489' + '57050582631942730706805009223062' + '73474534107340669624601458936165' + '97740410271692494532003787294341' + '70325843778659198143763193776859' + '86952408894019557734611984354530' + '15470437472077499697637500843089' + '26339295559968882457872412993810' + '12913029459299994792636526405928' + '46472097303849472116814344647144' + '38488520940127459844288859336526' + '896320919633919');
var DH_BASE = bigInt(2);

var DHKeyPair =
/*#__PURE__*/
function () {
  function DHKeyPair(isEphemeral) {
    var _this = this;

    _classCallCheck(this, DHKeyPair);
    ...
    if (isEphemeral) {
      ...
      var _loop = function _loop() {
        var randLen = Math.floor((_this.expBits + 7) / 8);
        var randArr = new Uint8Array(randLen);
        var lastByteBits = _this.expBits - (randLen - 1) * 8;
        window.crypto.getRandomValues(randArr);
        var mask = ~(-1 << lastByteBits);
        randArr[0] &= mask;
        _this.privKey = bigInt.fromArray(Array(randLen).fill(0).map(function (e, i) {
          return randArr[i];
        }), 256);
        valid = !(_this.privKey.compare(1) < 0 || _this.privKey.compare(DH_MODULUS.minus(2)) > 0 || _this.privKey.bitLength().compare(_this.expBits) !== 0);
      };
  ...
  _createClass(DHKeyPair, [{
    key: "getPublicEncoded",
    value: function getPublicEncoded() {
      var algoEnc = derEncodeOid(DH_DATA_BYTES);
      var paramsEnc = derEncodeInteger(DH_MODULUS);
      ...
      var keyEnc1 = derEncodeSequence(algoEnc);
      var keyEnc2 = derEncodeInteger(this.pubKey);
      keyEnc1 = keyEnc1.concat(derEncodeBitString(keyEnc2));
      var keyEncFinal = derEncodeSequence(keyEnc1);
      return keyEncFinal;
    }
  }, {
    key: "getPrivate",
    ...
    key: "getPrivateWrapped",
    value: function () {
      var _getPrivateWrapped = _asyncToGenerator(
      /*#__PURE__*/
      regeneratorRuntime.mark(function _callee(keyWrapper) {
        var toEncrypt, _ref, iv, hmac, ciphertext, privWrapped;
        ...
        return regeneratorRuntime.wrap(function _callee$(_context) {
          while (1) {
            switch (_context.prev = _context.next) {
              ...
              case 2:
                toEncrypt = this.getPrivate();
                _context.next = 5;
                return keyWrapper.encryptBytes(toEncrypt);
              ...
              case 5:
                ...
                return _context.abrupt("return", privWrapped);
      ...
      function getPrivateWrapped(_x) {
        return _getPrivateWrapped.apply(this, arguments);
      }

      return getPrivateWrapped;
    }()
  }, {
    key: "validateParty2PubKey",
    value: function validateParty2PubKey(inEnc) {
      // in ----------------------------------------------
      //  der                             key
      //   algId params                    key
      //          modulus base privBits?
      var inEncInterval = getDerInterval(inEnc, 0);
      var derEncInterval = getDerInterval(inEnc, inEncInterval.valIndex);
      var algIDInterval = getDerInterval(inEnc, derEncInterval.valIndex);
      var paramsEncInterval = getDerInterval(inEnc, algIDInterval.valEnd);
      var modulusInterval = getDerInterval(inEnc, paramsEncInterval.valIndex);
      var baseGenInterval = getDerInterval(inEnc, modulusInterval.valEnd);
      var keyEncInterval = getDerInterval(inEnc, derEncInterval.valEnd);
      var keyInterval = getDerInterval(inEnc, keyEncInterval.valIndex + 1);

      if (inEnc[inEncInterval.tagIndex] !== DER_TAGS.Sequence || inEnc[derEncInterval.tagIndex] !== DER_TAGS.Sequence || inEnc[algIDInterval.tagIndex] !== DER_TAGS.ObjectId || inEnc[paramsEncInterval.tagIndex] !== DER_TAGS.Sequence) {
        throw new Error('Error decoding X.509 encoded key');
      }
      ...
      var party2Modulus = getDerInteger(inEnc, modulusInterval);
      var party2BaseGen = getDerInteger(inEnc, baseGenInterval);

      if (DH_BASE.compare(party2BaseGen) !== 0 || DH_MODULUS.compare(party2Modulus) !== 0) {
        throw new Error('Invalid Diffie-Hellman parameters');
      }

      var party2PubKey = getDerInteger(inEnc, keyInterval);

      if (party2PubKey.compare(2) < 0 || party2PubKey.compare(DH_MODULUS.minus(2)) > 0 || DH_MODULUS.mod(party2PubKey) === 0) {
        throw new Error('Invalid Diffie-Hellman parameters');
      }
    ...
    key: "validateSelfPubKey",
    value: function validateSelfPubKey(inEnc) {
      ...
      var correctPubKey = DH_BASE.modPow(this.privKey, DH_MODULUS);
    ...
    value: function generateSecretKey(party2PubKey) {
      var expectedLen = Math.floor((DH_MODULUS.bitLength().value + 7) / 8);
      var secretKey = party2PubKey.modPow(this.privKey, DH_MODULUS);
    ...
    key: "fromSerialized",
    value: function () {
      var _fromSerialized = _asyncToGenerator(
      /*#__PURE__*/
      regeneratorRuntime.mark(function _callee2(privWrapped, pubX509, keyWrapper) {
        var _privWrapped$split, _privWrapped$split2, privIV, privHMAC, privCiphertext, pubX509Bytes, pair, privTypedArr, privArr, i;

        return regeneratorRuntime.wrap(function _callee2$(_context2) {
          while (1) {
            switch (_context2.prev = _context2.next) {
              case 0:
                _privWrapped$split = privWrapped.split(';'), _privWrapped$split2 = _slicedToArray(_privWrapped$split, 3), privIV = _privWrapped$split2[0], privHMAC = _privWrapped$split2[1], privCiphertext = _privWrapped$split2[2];
              ...
              case 3:
                ...
                pubX509Bytes = base64decodebytes(pubX509);
                pair = new DHKeyPair(false);
              ...
              case 12:
                if (!(privCiphertext.findIndex(function (e) {
              ...
              case 14:
                if (!(pubX509Bytes.findIndex(function (e) {
                  return !Number.isInteger(e) || e < 0 || e > 255;
                }) !== -1)) {
                  _context2.next = 16;
                  break;
              ...
              case 16:
                _context2.next = 18;
                return keyWrapper.decryptBytes(privIV, privHMAC, privCiphertext);

              case 18:
                privTypedArr = _context2.sent;
                privArr = new Array(privTypedArr.length);

                for (i = 0; i < privTypedArr.length; i++) {
                  privArr[i] = privTypedArr[i];
                }

                pair.privKey = bigInt.fromArray(privArr, 256);

                if (!(pair.privKey.compare(1) < 0 || pair.privKey.compare(DH_MODULUS.minus(2)) > 0 || pair.privKey.bitLength().compare(pair.expBits) !== 0)) {
                  _context2.next = 24;
                  break;
                }
      ...
      function fromSerialized(_x2, _x3, _x4) {
...
var assertSubtle = false;

var CipherStore =
/*#__PURE__*/
function () {
  function CipherStore(keyBytes) {
    var _this2 = this;

    var initializeIV = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;

    _classCallCheck(this, CipherStore);
    ...
    if (initializeIV) {
      ...
      var ivView = new Uint8Array(this.iv);
  ...
  _createClass(CipherStore, [{
    key: "getParamsEncoded",
    value: function getParamsEncoded() {
      if (empty(this.iv)) {
    ...
    key: "setParamsRandom",
    value: function setParamsRandom() {
      this.iv = new ArrayBuffer(16);
      var ivView = new Uint8Array(this.iv);
      window.crypto.getRandomValues(ivView);
        ...
        return regeneratorRuntime.wrap(function _callee3$(_context3) {
          while (1) {
            switch (_context3.prev = _context3.next) {
              ...
              case 2:
                toEncrypt = toUTF8Bytes(str);
                return _context3.abrupt("return", this.encryptBytes(toEncrypt));
    ...
    key: "encryptBytes",
    value: function () {
      var _encryptBytes = _asyncToGenerator(
      /*#__PURE__*/
      regeneratorRuntime.mark(function _callee4(b256) {
        var ivView, ivArr, encBuffer, encBufferView, outBuffer, outBufferView, keyMatView, hmac;
        return regeneratorRuntime.wrap(function _callee4$(_context4) {
          while (1) {
            switch (_context4.prev = _context4.next) {
              case 0:
                this.setParamsRandom();
                ivView = new Uint8Array(this.iv);
                ivArr = new Array(16);
                typedArrayCopy(ivView, 0, ivArr, 0, 16);
                encBuffer = new ArrayBuffer(b256.length);
                encBufferView = new Uint8Array(encBuffer);
              ...
              case 9:
                outBuffer = _context4.sent;
                outBufferView = new Uint8Array(outBuffer);
                keyMatView = new Uint8Array(this.keyMat);
                hmac = {};
                hmac.b = new ArrayBuffer(64 + 16 + outBufferView.length);
                hmac.bView = new Uint8Array(hmac.b);
                typedArrayCopyAndMap(keyMatView, 0, hmac.bView, 0, 32, function (byte) {
                  return byte ^ 0x36;
                });
    ...
    value: function () {
      ...
      /*#__PURE__*/
      regeneratorRuntime.mark(function _callee5(ivArr, hmacArr, b256e) {
        var b256;
        return regeneratorRuntime.wrap(function _callee5$(_context5) {
          while (1) {
            switch (_context5.prev = _context5.next) {
              case 0:
                _context5.next = 2;
                return this.decryptBytes(ivArr, hmacArr, b256e);

              case 2:
                b256 = _context5.sent;
                return _context5.abrupt("return", fromUTF8Bytes(b256));
    ...
    key: "decryptBytes",
    value: function () {
      var _decryptBytes = _asyncToGenerator(
      /*#__PURE__*/
      regeneratorRuntime.mark(function _callee6(ivArr, hmacArr, b256e) {
        var keyMatView, hmac, i, iv, ivView, decBuffer, decBufferView, outBuffer, outBufferView;
        return regeneratorRuntime.wrap(function _callee6$(_context6) {
          while (1) {
            switch (_context6.prev = _context6.next) {
              ...
              case 21:
                hmac.result = _context6.sent;
                hmac.resultView = new Uint8Array(hmac.result);
                i = 0;

              case 24:
                if (!(i < hmac.resultView.length)) {
                  _context6.next = 30;
                  break;
                }
              ...
              case 30:
                ...
                ivView = new Uint8Array(iv);
                typedArrayCopy(ivArr, 0, ivView, 0, 16);
                decBuffer = new ArrayBuffer(b256e.length);
                decBufferView = new Uint8Array(decBuffer);
              ...
              case 38:
                outBuffer = _context6.sent;
                outBufferView = new Uint8Array(outBuffer);
                return _context6.abrupt("return", outBufferView);
      ...
      function decryptBytes(_x10, _x11, _x12) {
...
function tripleKeyAgree(_x13, _x14, _x15, _x16) {
  return _tripleKeyAgree.apply(this, arguments);
}

function _tripleKeyAgree() {
  _tripleKeyAgree = _asyncToGenerator(
  /*#__PURE__*/
  regeneratorRuntime.mark(function _callee12(selfSerializedKeys, otherSerializedKeys, party1, keyWrapper) {
    var keyAgreement, selfKeys, otherKeys, sharedSecretBufArr, sharedSecret, sharedSecretView, j, i, bufArrView, ji, keyMat, keyMatV, hash, hashV, _i2;
      ...
      while (1) {
        switch (_context12.prev = _context12.next) {
          ...
          case 3:
            ...
            sharedSecretBufArr = [];
            _context12.next = 8;
            return DHKeyPair.fromSerialized(selfSerializedKeys.identity_private, selfSerializedKeys.identity_public, keyWrapper);

          case 8:
            selfKeys.identity = _context12.sent;
            ...
            _context12.next = 12;
            return DHKeyPair.fromSerialized(selfSerializedKeys.prekey_private, selfSerializedKeys.prekey_public, keyWrapper);

          case 12:
            selfKeys.prekey = _context12.sent;
            _context12.next = 17;
            break;
          ...
          case 17:
            ...
            sharedSecret = new ArrayBuffer(sharedSecretBufArr[0].byteLength + sharedSecretBufArr[1].byteLength + sharedSecretBufArr[2].byteLength);
            sharedSecretView = new Uint8Array(sharedSecret);
            j = 0;

            for (i = 0; i < 3; i++) {
              bufArrView = new Uint8Array(sharedSecretBufArr[i]);

              for (ji = j; j < ji + bufArrView.length; j++) {
                sharedSecretView[j] = bufArrView[j - ji];
              }
            }

            keyMat = new ArrayBuffer(16);
            keyMatV = new Uint8Array(keyMat);
            _context12.next = 27;
            return window.crypto.subtle.digest('sha-256', sharedSecret);
          ...
          case 27:
            ...
            for (_i2 = 0; _i2 < 16; _i2++) {
              keyMatV[_i2] = hashV[_i2];
            }

            keyAgreement.key_secret = keyMat;
            return _context12.abrupt("return", keyAgreement);
...
function wrapKey(_x17, _x18) {
  return _wrapKey.apply(this, arguments);
}

function _wrapKey() {
  _wrapKey = _asyncToGenerator(
  /*#__PURE__*/
  regeneratorRuntime.mark(function _callee13(keyBytes, keyWrapper) {
    var _ref5, iv, hmac, ciphertext, keyWrapped;
...
function _unwrapKey() {
  ...
  /*#__PURE__*/
  regeneratorRuntime.mark(function _callee14(keyWrapped, keyWrapper) {
    var _keyWrapped$split, _keyWrapped$split2, iv, hmac, ciphertext, decrypted;
    ...
    return regeneratorRuntime.wrap(function _callee14$(_context14) {
      while (1) {
        switch (_context14.prev = _context14.next) {
          ...
          case 2:
            _keyWrapped$split = keyWrapped.split(';'), _keyWrapped$split2 = _slicedToArray(_keyWrapped$split, 3), iv = _keyWrapped$split2[0], hmac = _keyWrapped$split2[1], ciphertext = _keyWrapped$split2[2];
          ...
          case 5:
            ...
            _context14.next = 10;
            return keyWrapper.decryptBytes(iv, hmac, ciphertext);

          case 10:
            decrypted = _context14.sent;
            return _context14.abrupt("return", decrypted);
...
var commandHandlers = {
  conversation_request(commandResults, session) {
    var conversationRequest = JSON.parse(commandResults);
    session.addConversationFromRequest(conversationRequest).then(function (toAdd) {
      session.enqueue('conversation_add ' + JSON.stringify(toAdd), 0);
    });
  },

  conversation_ls(commandResults, session) {
    var conversationLS = JSON.parse(commandResults);
    var keysToRequest = [];
    var promises = [];
    conversationLS.forEach(function (conversationObj) {
      var promise = session.addConversationFromLS(conversationObj).then(function (conversationStatus) {
        if (conversationStatus === false) {
          ...
          session.enqueue("conversation_set_key ".concat(conversationObj.id, " ").concat(conversationStatus), 0);
        }
      });
  ...
  conversation_cat(commandResults, session) {
    ...
    if (pastMessages.length > 0) {
      var conversationID = parseInt(pastMessages[0].split(';')[0]);
      var conversation = session.conversations.find(function (e) {
      ...
      pastMessages.forEach(function (pastMsg) {
        ...
        var msgFields = pastMsg.split(';', 6);

        var _msgFields = _slicedToArray(msgFields, 6),
            id = _msgFields[0],
            from = _msgFields[1],
            time = _msgFields[2],
            contentType = _msgFields[3],
            iv = _msgFields[4],
            hmac = _msgFields[5];

        if (empty(id, 'string') || empty(from, 'string') || empty(time, 'string') || empty(contentType, 'string') || empty(iv, 'string') || empty(hmac, 'string')) {
          return;
        }
        ...
        var ciphertext = pastMsg.substring(msgFields.reduce(function (acc, cur) {
          return acc + cur.length;
        }, 0) + 6);
        msgObj.from = from;
        msgObj.time = parseInt(time);
        msgObj.contentType = contentType;
        conversation.cipher.decrypt(base64decodebytes(iv), base64decodebytes(hmac), base64decodebytes(ciphertext)).then(function (msgData) {
          msgObj.data = msgData;
          var msgPushed = false;

          if (!empty(session.externalMessageHandlers, 'object') && !empty(session.externalMessageHandlers.conversation_cat, 'function')) {
  ...
  retrieve_keys_other(commandResults, session) {
    ...
    otherKeysets.forEach(function (keyset) {
      session.keysets.push(keyset);
      var conversationObj = session.pendingConversations.find(function (e) {
        return keyset.user === e.role1;
      });

      if (conversationObj != null) {
        session.addConversationFromLS(conversationObj).then(function (conversationStatus) {
          if (conversationStatus === false) {
            throw new Error('retrieve_keys_self must be called before retrieve_keys_other');
          } else if (conversationStatus === true) {
            if (session.catSent.indexOf(conversationObj.id) === -1) {
              session.catSent.push(conversationObj.id);
              session.pendingConversations = session.pendingConversations.filter(function (e) {
                return keyset.user !== e.role1;
              });
              session.enqueue("conversation_cat ".concat(conversationObj.id), 0);
            }
  ...
  set_preferences(commandResults, session) {
    var preferences = JSON.parse(commandResults);

    if (!empty(session.externalMessageHandlers, 'object') && !empty(session.externalMessageHandlers.set_preferences, 'function')) {
  ...
  user_message(currentMsg, session) {
    ...
    var msgObj = {};
    var msgFields = currentMsg.split(';', 6);
    var conversationID = parseInt(msgFields[0]);

    var _msgFields$slice = msgFields.slice(1),
        _msgFields$slice2 = _slicedToArray(_msgFields$slice, 5),
        from = _msgFields$slice2[0],
        time = _msgFields$slice2[1],
        contentType = _msgFields$slice2[2],
        iv = _msgFields$slice2[3],
        hmac = _msgFields$slice2[4];

    if (empty(from, 'string') || empty(time, 'string') || empty(contentType, 'string') || empty(iv, 'string') || empty(hmac, 'string')) {
      return;
    }
    ...
    conversation.cipher.decrypt(base64decodebytes(iv), base64decodebytes(hmac), base64decodebytes(ciphertext)).then(function (msgData) {
      ...
      if (!empty(session.externalMessageHandlers, 'object') && !empty(session.externalMessageHandlers.user_message, 'function')) {
...
/*#__PURE__*/
function () {
  function ChatSession(conn, externalMessageHandlers) {
    var _this3 = this;
    ...
    this.websocket = new WebSocket("wss://".concat(window.location.hostname, ":8082/").concat(this.uuid));

    this.websocket.onmessage = function (m) {
      _this3.internalMessageHandler(m.data);
    };
  }

  _createClass(ChatSession, [{
    key: "enqueueWithContentType",
    value: function () {
      var _enqueueWithContentType = _asyncToGenerator(
      /*#__PURE__*/
      regeneratorRuntime.mark(function _callee7(str, contentType, conversationID) {
        var _this4 = this;

        var msg, conv, _ref2, iv, hmac, ciphertext, _msg;
            ...
            switch (_context7.prev = _context7.next) {
              ...
              case 2:
                ...
                return new Promise(function (resolve, reject) {
                  _this4.websocket.onopen = resolve;
                });
              ...
              case 15:
                ...
                conv = this.conversations.find(function (e) {
                  return e.id === conversationID;
                });
              ...
              case 19:
                _context7.next = 21;
                return conv.cipher.encrypt(str);

              case 21:
                _ref2 = _context7.sent;
                iv = _ref2.iv;
                hmac = _ref2.hmac;
                ciphertext = _ref2.ciphertext;
                _msg = "".concat(conversationID, ";").concat(contentType, ";");
                _msg += base64encodebytes(iv) + ';';
                _msg += base64encodebytes(hmac) + ';';
                _msg += base64encodebytes(ciphertext);
                this.websocket.send(_msg);
                return _context7.abrupt("return", true);
    ...
    value: function () {
      ...
      /*#__PURE__*/
      regeneratorRuntime.mark(function _callee8(str, conversationID) {
        var result;
        return regeneratorRuntime.wrap(function _callee8$(_context8) {
          while (1) {
            switch (_context8.prev = _context8.next) {
              case 0:
                _context8.next = 2;
                return this.enqueueWithContentType(str, '', conversationID);
    ...
    key: "addConversationFromRequest",
    value: function () {
      var _addConversationFromRequest = _asyncToGenerator(
      /*#__PURE__*/
      regeneratorRuntime.mark(function _callee10(conversationRequest) {
        var _this5 = this;

        var self, toAdd, userNameList, conversationKeyBytes, conversationCipher, asyncLoopFunction, promises, selfUserObj;
        return regeneratorRuntime.wrap(function _callee10$(_context10) {
          while (1) {
            switch (_context10.prev = _context10.next) {
              case 0:
                self = conversationRequest.find(function (e) {
                  return 'identity_private' in e;
                });
              ...
              case 3:
                ...
                userNameList = [self.user];
                conversationKeyBytes = new Uint8Array(32);
                window.crypto.getRandomValues(conversationKeyBytes);
                conversationCipher = new CipherStore(conversationKeyBytes);
                _context10.next = 10;
                return conversationCipher.readyPromise;
              ...
              case 10:
                ...
                function () {
                  ...
                  /*#__PURE__*/
                  regeneratorRuntime.mark(function _callee9(other) {
                    var keyAgreement, otherUserObj, initialCipher, _ref4, iv, hmac, ciphertext;
                      ...
                      while (1) {
                        switch (_context9.prev = _context9.next) {
                          ...
                          case 2:
                            ...
                            _context9.next = 5;
                            return tripleKeyAgree(self, other, true, _this5.keyWrapper);

                          case 5:
                            keyAgreement = _context9.sent;
                            otherUserObj = {
                              user: other.user,
                              role: 2,
                            ...
                            initialCipher = new CipherStore(keyAgreement.key_secret);
                            _context9.next = 10;
                            return initialCipher.readyPromise;

                          case 10:
                            _context9.next = 12;
                            return initialCipher.encryptBytes(conversationKeyBytes);

                          case 12:
                            _ref4 = _context9.sent;
                            iv = _ref4.iv;
                            hmac = _ref4.hmac;
                            ciphertext = _ref4.ciphertext;
                ...
                promises = [];
                conversationRequest.forEach(function (other) {
              ...
              case 15:
                ...
                _context10.next = 18;
                return wrapKey(conversationKeyBytes, this.keyWrapper);

              case 18:
                selfUserObj.key_wrapped = _context10.sent;
                toAdd.users.push(selfUserObj);
                return _context10.abrupt("return", toAdd);
      ...
      return addConversationFromRequest;
    }()
  }, {
    key: "addConversationFromLS",
    value: function () {
      var _addConversationFromLS = _asyncToGenerator(
      /*#__PURE__*/
      regeneratorRuntime.mark(function _callee11(conversationObj) {
        var toAdd, conversationKeyBytes, self, other, _conversationObj$init, _conversationObj$init2, iv, hmac, ciphertext, otherUserObj, keyAgreement, initialCipher, _conversationKeyBytes, conversationKeyWrapped;
          ...
          while (1) {
            switch (_context11.prev = _context11.next) {
              case 0:
                ...
                if (!(this.conversations.findIndex(function (e) {
                  return e.id === conversationObj.id;
                }) !== -1)) {
                  _context11.next = 4;
                  break;
                }
              ...
              case 4:
                _context11.next = 6;
                return unwrapKey(conversationObj.key_wrapped, this.keyWrapper);

              case 6:
                conversationKeyBytes = _context11.sent;
                toAdd.cipher = new CipherStore(conversationKeyBytes);
                _context11.next = 10;
                return toAdd.cipher.readyPromise;
              ...
              case 14:
                ...
                self = this.keysets.find(function (e) {
                  return 'identity_private' in e;
                });
                other = this.keysets.find(function (e) {
                  return e.user === conversationObj.role1;
                });
              ...
              case 18:
                _conversationObj$init = conversationObj.initial_message.split(';'), _conversationObj$init2 = _slicedToArray(_conversationObj$init, 3), iv = _conversationObj$init2[0], hmac = _conversationObj$init2[1], ciphertext = _conversationObj$init2[2];
              ...
              case 21:
                otherUserObj = Object.assign({}, other, {
                  ephemeral_public: conversationObj.key_ephemeral_public
                });
                _context11.next = 24;
                return tripleKeyAgree(self, otherUserObj, false, this.keyWrapper);

              case 24:
                keyAgreement = _context11.sent;
                initialCipher = new CipherStore(keyAgreement.key_secret);
                _context11.next = 28;
                return initialCipher.readyPromise;
              ...
              case 28:
                _context11.next = 30;
                return initialCipher.decryptBytes(base64decodebytes(iv), base64decodebytes(hmac), base64decodebytes(ciphertext));

              case 30:
                _conversationKeyBytes = _context11.sent;
                toAdd.cipher = new CipherStore(_conversationKeyBytes);
                _context11.next = 34;
                return toAdd.cipher.readyPromise;
              ...
              case 34:
                this.conversations.push(toAdd);
                _context11.next = 37;
                return wrapKey(_conversationKeyBytes, this.keyWrapper);

              case 37:
                conversationKeyWrapped = _context11.sent;
                return _context11.abrupt("return", conversationKeyWrapped);
      ...
      return addConversationFromLS;
    }()
  }, {
    key: "internalMessageHandler",
    value: function internalMessageHandler(message) {
      console.log(message);

      var _message$split = message.split(';', 2),
          _message$split2 = _slicedToArray(_message$split, 2),
          conversationID = _message$split2[0],
          messageType = _message$split2[1];
      ...
      if (conversationID === 0) {
        ...
        if (messageType === 'command') {
          var messageData = message.substring(conversationID.toString().length + messageType.length + 2);
          var commandIdx = messageData.indexOf(';');
          var command = messageData.substring(0, commandIdx);
          var commandResults = messageData.substring(commandIdx + 1);
...
function chatClientBegin(externalMessageHandlers) {
  return new Promise(function (resolve2, reject2) {
    new Promise(function (resolve, reject) {
      ...
      var ref = new ChatSession(uuid, externalMessageHandlers);
      ref.keyWrapper = new CipherStore(base64decodebytes(localStorage.getItem('keyWrapper')), false);
      ref.keyWrapper.readyPromise.then(function () {
        ref.enqueue('retrieve_keys_self', 0).then(function () {
          return ref.enqueue('conversation_ls', 0);
        }).then(function () {
          return resolve2(ref);
        });
      });
...
var regexpFlags = __webpack_require__(12);
...
if (__webpack_require__(3)) {
  ...
  var createArrayIncludes = __webpack_require__(30);
  var speciesConstructor = __webpack_require__(58);
